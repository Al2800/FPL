...
if (-not $CodexPath) {
    ...
    if (-not $command) { throw 'The Codex CLI was not found on PATH.' }
...
function Register-AgentTask {
    param([string]$TaskName, [string]$Time, [string]$TaskId, [string]$PromptPath)
    ...
    if ($taskCommand.Length -gt 261) { throw "Task Scheduler command is too long: $($taskCommand.Length) characters." }
