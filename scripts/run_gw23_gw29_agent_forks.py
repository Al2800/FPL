#!/usr/bin/env python3
"""Prepare and run the sequential GW23-GW29 Sol evidence trajectory."""
...
def _state(gameweek: int, previous_version: str) -> dict:
    ...
    raise ValueError("Only GW23-GW29 are supported")
...
def _prepare(
    ...
    return {
        ...
        "request_sha256": (
            bundle["evidence_request"]["rendered_input_sha256"]
        ),
    }
...
def _complete_week(
    ...
    comparison, _, _ = run_sequential_agent_fork_week(
        ...
        transition_to_next=gameweek < 29,
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--gameweek", type=int, choices=range(23, 30), required=True
    )
