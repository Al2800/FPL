#!/usr/bin/env python3
"""Prepare and run the sequential GW15-GW17 Sol evidence forks."""
...
def _state(gameweek: int) -> dict:
    if gameweek == 15:
        state, _ = derive_next_state_from_agent_fork(
            ...
            fork_root=_output(14),
        )
        return state
    if gameweek in (16, 17):
        return _read(_output(gameweek - 1) / "next-policy-state.json")
    raise ValueError("Only GW15, GW16 and GW17 are supported")
...
def _complete_week(gameweek: int) -> dict:
    ...
    comparison, _, _ = run_sequential_agent_fork_week(
        ...
        transition_to_next=gameweek < 17,
    )
...
def main() -> int:
    ...
    parser.add_argument("--gameweek", type=int, choices=(15, 16, 17), default=15)
    ...
    if args.mode == "prepare":
        ...
        result = {
            "gw15": _complete_week(15),
            "gw16": _complete_week(16),
            "gw17": _complete_week(17),
        }
