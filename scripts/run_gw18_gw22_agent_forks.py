...
def _state(gameweek: int) -> dict:
    if gameweek == 18:
        state, _ = derive_next_state_from_agent_fork(
            ...
            fork_root=_output(17),
        )
        return state
    if 19 <= gameweek <= 22:
        return _read(_output(gameweek - 1) / "next-policy-state.json")
...
def _prepare(gameweek: int) -> dict:
    ...
    path = REPO / f"evals/evidence-forks/2025-26/gw-{gameweek:02d}/agent-host-bundle.json"
...
def _complete_week(gameweek: int) -> dict:
    ...
    comparison, _, _ = run_sequential_agent_fork_week(
        gameweek=gameweek, state=_state(gameweek), host_bundle=bundle,
        evidence_run=evidence_run, challenger_run=challenger_run,
        canonical_root=CANONICAL, episode_root=EPISODES,
        output_root=_output(gameweek), transition_to_next=gameweek < 22,
    )
    attribution = build_same_state_control_attribution(
        gameweek=gameweek, state=_state(gameweek), canonical_root=CANONICAL,
        episode_root=EPISODES, agent_output_root=_output(gameweek),
    )
...
def main() -> int:
    ...
    if args.mode == "prepare":
        ...
        result = {f"gw{gw}": _complete_week(gw) for gw in range(18, 23)}
